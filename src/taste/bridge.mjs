...
export function tasteCascadeBridge(taste, cascade) {
  ...
  if (steering) {
    ...
    if (steering.seLegacy > 0.95 && cascade.yield < 500) {
      insights.push({
        ...
        cascadeImpact:
          "You accept almost everything the agent produces, but your cascade isn't compounding. High acceptance means low intervention — you're not steering the agent toward higher-yield output.",
        recommendation:
          "Push the agent to produce more output per turn. Your steering is efficient but your velocity is the bottleneck. Ask for complete implementations, not incremental pieces.",
        severity: "medium",
      });
    }
    ...
    if (overCorr && overCorr.value > 0.3) {
      insights.push({
        ...
        cascadeImpact:
          "Each unnecessary correction costs ~2K input tokens (re-reading the file + re-instructing the agent). Over 500 sessions, that's wasted input that inflates your denominator and suppresses Υ.",
        recommendation:
          "Before correcting, ask: am I fixing the agent's output, or adding to it? If adding, let the agent do it in the next turn instead of correcting manually.",
        severity: "medium",
      });
    }
    ...
    if (timing && timing.value !== null && timing.value < 5) {
      insights.push({
        ...
        cascadeImpact:
          "Very tight steering can prevent the agent from completing multi-step work. Each interruption resets the agent's context, potentially causing cache misses and re-reads.",
        recommendation:
          "Let the agent finish a complete unit of work before intervening. Batch your feedback instead of correcting each step.",
        severity: "low",
      });
    }
    ...
    if (reliance && reliance.value !== null && reliance.value < -0.01) {
      insights.push({
        ...
        cascadeImpact:
          "Decreasing reliance may indicate the agent is degrading, or you're becoming more cautious. Either way, more corrections = more input tokens = lower Υ.",
        recommendation:
          "Check if the agent's output quality actually changed, or if your standards shifted. If the agent is fine, your extra corrections are pure token waste.",
        severity: "low",
      });
    }
  }
  ...
  if (fingerprint && fingerprint.length > 0) {
    ...
    if (allTotal > 0 && docsTotal / allTotal > 0.3) {
      insights.push({
        ...
        cascadeImpact:
          "Documentation edits generate output tokens but don't improve your cascade architecture. Υ rewards output that compounds (code, not docs). Your output is going to non-compounding work.",
        recommendation:
          "Delegate documentation to the agent in larger batches. Spend your steering on production code where output quality directly affects Υ.",
        severity: "low",
      });
    }

    // Diverging correction loops = agent struggling, burning tokens
    const diverging = fingerprint.filter(
      (f) => f.diverging && f.correctionLoops > 2,
    );
    if (diverging.length > 0) {
      insights.push({
        ...
        cascadeImpact:
          "Diverging loops are the most expensive token waste — each iteration produces MORE output that needs MORE correction. The agent isn't matching your taste on these files.",
        recommendation:
          "For diverging files, stop the loop and give explicit taste guidance. Feed your taste profile at session start. If a file diverges past 5 edits, start fresh with a clearer prompt.",
        severity: "high",
      });
    }
    ...
    if (taste.iterationConcentration === "concentrated" && fingerprint[0]) {
      insights.push({
        ...
        cascadeImpact:
          "Concentrated iteration on one file means your output tokens are going to a single surface. If that surface is production code, this is fine. If it's polish/design, you're over-investing output in one area.",
        recommendation:
          "Check if this file is production-critical. If not, consider whether the iteration depth is justified or if you're perfectionism-cycling.",
        severity: "low",
      });
    }
  }
  ...
  if (rhythm) {
    ...
    if (rhythm.toolDistribution.bash > 40) {
      insights.push({
        ...
        recommendation:
          "Use Read/Grep to verify before Bash. Reading is cache-friendly (reuses context); running generates new output that displaces cache.",
        severity: "medium",
      });
    }
    ...
    if (rhythm.investigateToEdit < 0.5) {
      insights.push({
        ...
        cascadeImpact:
          "Editing without investigating leads to correction loops (the agent doesn't have enough context). Each correction loop costs input tokens for re-instruction.",
        recommendation:
          "Read the file before editing. Let the agent investigate (Read/Grep) before it edits. The cache cost of reading is lower than the input cost of correcting.",
        severity: "medium",
      });
    }
    ...
    if (rhythm.askUserFrequency.calls === 0 && cascade.velocity < 1) {
      insights.push({
        ...
        cascadeImpact:
          "The agent is running autonomously but not producing enough output. Without your direction, it may be cautious — generating less output per turn.",
        recommendation:
          'Give the agent explicit output targets: "implement the full feature, not just the skeleton." Autonomy works best when the agent knows what "done" looks like.',
        severity: "low",
      });
    }
  }
  ...
  if (personality) {
    ...
    if (
      personality.types.includes("cache-hoarder") &&
      personality.types.includes("low-velocity")
    ) {
      insights.push({
        ...
        cascadeImpact:
          "You're excellent at reusing context (high leverage) but you're not converting that context into output. Υ = (cacheRead × output) / input². Your cacheRead is high but your output is low — the formula punishes you for not producing.",
        recommendation:
          "Push the agent to generate more. Ask for complete implementations, not pieces. Your cache leverage is your strength — use it to produce more output, not to hoard more context.",
        severity: "high",
      });
    }
    ...
    if (personality.types.includes("input-heavy")) {
      insights.push({
        ...
        cascadeImpact:
          "Fresh input is the most expensive token type. The cascade rewards cache reuse (low input, high cacheRead). High input suppresses leverage and SNR.",
        recommendation:
          "Compress your prompts. Reference prior context instead of re-explaining. Let the agent read files instead of pasting content into the prompt.",
        severity: "high",
      });
    }
    ...
    if (personality.types.includes("output-light")) {
      insights.push({
        ...
        cascadeImpact:
          "Υ = (cacheRead × output) / input². Output is in the numerator — low output directly suppresses Υ. You're spending tokens on input and cache but not producing results.",
        recommendation:
          "Ask the agent for more: complete files, full implementations, detailed analysis. Each output token contributes to Υ; each input token detracts.",
        severity: "high",
      });
    }
  }
  ...
  if (taxonomy && taxonomy.length > 0) {
    ...
    if (
      ["design", "ui-component", "styling"].includes(topCat.category) &&
      topCat.totalLoops > 10
    ) {
      insights.push({
        ...
        recommendation:
          'Feed your design preferences to the agent at session start. Use the taste profile. Be explicit about design rules: "no median, large numbers, gold accent." Reduce the loop depth by front-loading taste.',
        severity: "high",
      });
    }
    ...
    if (topCat.category === "code" && topCat.totalLoops > 5) {
      insights.push({
        ...
        cascadeImpact:
          "Code corrections indicate the agent is producing buggy or wrong logic. Unlike design corrections, these should converge (the agent learns from the fix). If they're not converging, the agent may be in the wrong context.",
        recommendation:
          "Check if the agent has enough context before it starts coding. Read the relevant files first. If loops diverge, stop and re-explain the requirement.",
        severity: "medium",
      });
    }
  }
...
export function formatBridgeReport(insights) {
  ...
  lines.push("");
  lines.push(
    `${insights.length} insight${insights.length === 1 ? "" : "s"} connecting your behavior to your cascade:`,
  );
  lines.push("");
  for (const ins of insights) {
    ...
    lines.push(
      `${icon} [${ins.severity.toUpperCase()}] ${ins.dimension.toUpperCase()}`,
    );
