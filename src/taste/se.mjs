...
export function appropriateSteeringIndex(agg) {
  ...
  for (const turn of turns) {
    ...
    correctionsAnalyzed++
    const assistantHadError = turn.assistantMsg?.toolResults?.some(tr => tr.isError && !isRejection(tr.content))
    ...
    const avgCorrectionDelta = correctionDeltas.length > 0
  ...
  const correctionPrecision = correctionsAnalyzed > 0 ? likelyNeeded / correctionsAnalyzed : null
  ...
  for (const turn of turns) {
    if (turn.corrected || turn.rejected) {
      ...
      if (aTs && uTs) {
        const deltaSec = (new Date(uTs).getTime() - new Date(aTs).getTime()) / 1000
  ...
  const timingLabel = medianTiming === null ? 'unknown'
    : medianTiming < 10 ? 'very tight (< 10s)'
    : medianTiming < 30 ? 'tight (10-30s)'
    : medianTiming < 120 ? 'moderate (30s-2min)'
    : 'loose (> 2min)'
  ...
  for (let i = 1; i < perSession.length; i++) {
    if (perSession[i].acceptanceRate !== null && perSession[i - 1].acceptanceRate !== null) {
      slopes.push(perSession[i].acceptanceRate - perSession[i - 1].acceptanceRate)
    }
  }
  const relianceSlope = slopes.length > 0
    ? slopes.reduce((a, b) => a + b, 0) / slopes.length
    : null
  const relianceTrend = relianceSlope === null ? 'unknown'
    : relianceSlope > 0.01 ? 'increasing (trusting agent more)'
    : relianceSlope < -0.01 ? 'decreasing (losing confidence)'
    : 'stable'
  ...
  const overCorrectionIndex = correctionsAnalyzed > 0 ? possiblyUnnecessary / correctionsAnalyzed : null
  ...
  const underSteeringIndex = acceptedTurns > 0 ? downstreamCorrections / acceptedTurns : null
  ...
  if (correctionPrecision !== null && overCorrectionIndex !== null && underSteeringIndex !== null) {
    const w = {
      acceptance: 0.15,
      correction: 0.15,   // lower correction rate is better, but not too low
      rejection: 0.15,    // moderate rejection is appropriate
      precision: 0.20,    // higher precision = better steering
      overCorrection: 0.15, // lower over-correction = better
      underSteering: 0.10,  // lower under-steering = better
    }
    ...
    const appropriateCorrection = Math.max(0, 1 - Math.abs(correctionRate - 0.10) * 2)
    // Rejection: moderate is good. 0% = under-steering. >5% = over-rejecting.
    const appropriateRejection = Math.max(0, 1 - Math.abs(rejectionRate - 0.02) * 5)
  ...
  return {
    asi: asiComposite,
    seLegacy: round((1.0 * accepted + 0.5 * corrected + 0.0 * rejected) / total, 4),
    dimensions: {
      ...
      correctionPrecision: { value: correctionPrecision !== null ? round(correctionPrecision, 4) : null, confidence: 'medium' },
      interventionTiming: { value: medianTiming !== null ? round(medianTiming, 1) : null, label: timingLabel, confidence: 'high' },
      relianceSlope: { value: relianceSlope !== null ? round(relianceSlope, 4) : null, trend: relianceTrend, confidence: 'high' },
      overCorrectionIndex: { value: overCorrectionIndex !== null ? round(overCorrectionIndex, 4) : null, confidence: 'low' },
      underSteeringIndex: { value: underSteeringIndex !== null ? round(underSteeringIndex, 4) : null, confidence: 'medium' },
    },
...
export function perSessionASI(sessions) {
  ...
  for (const s of sessions) {
    ...
    results.push({
      ...
      correctionRate: round(corrected / total, 4),
      rejectionRate: round(rejected / total, 4),
      accepted, corrected, rejected, total,
    })
  }
  return results
}
...
const REJECTION_LANG = /\b(no|wrong|revert|undo|that'?s not right|not what i want|don't do that|stop|fix this|try again|redo)\b/i
