...
export function extractTaste(agg, operator = "local", opts = {}) {
  ...
  const iterationFingerprint = fileEntries.map(([file, count]) => {
    ...
    const maxDepth =
      loops.length > 0 ? Math.max(...loops.map((l) => l.depth)) : 0;
    return {
      file: file.split("/").pop(),
      fullPath: file,
      editCount: count,
  ...
  const topFileShare =
    fileEntries.length > 0 ? fileEntries[0][1] / totalEdits : 0;
  const top3Share =
    fileEntries.slice(0, 3).reduce((sum, [, c]) => sum + c, 0) / totalEdits;
  const iterationConcentration =
    top3Share > 0.5
      ? "concentrated"
      : top3Share > 0.3
        ? "moderate"
  ...
  const toolEntries = Object.entries(agg.toolNameCounts).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  const editCount =
    (agg.toolNameCounts.Edit || 0) +
    (agg.toolNameCounts.MultiEdit || 0) +
    (agg.toolNameCounts.Write || 0);
  const bashCount = agg.toolNameCounts.Bash || 0;
  const grepCount =
    (agg.toolNameCounts.Grep || 0) + (agg.toolNameCounts.Glob || 0);
  ...
  const workflowRhythm = {
    primaryTool: topTool
      ? { name: topTool[0], share: round((topTool[1] / totalTools) * 100, 1) }
      : null,
    toolDistribution: {
    ...
    style:
      investigateToEdit > 1.5
        ? "investigate-first (explores before editing)"
        : bashToEdit > 2
          ? "trial-and-error (tests by running)"
          : editCount > investigateCount
            ? "iterative (rapid edit cycles)"
            : "balanced",
    askUserFrequency:
      agg.totalAskUser > 0
        ? {
            calls: agg.totalAskUser,
            perTurn: round((agg.totalAskUser / (agg.totalTurns || 1)) * 100, 1),
          }
        : {
            calls: 0,
            perTurn: 0,
            note: "prefers autonomy (zero AskUserQuestion calls)",
          },
    sessionShape:
      agg.sessionCount > 0
        ? {
            count: agg.sessionCount,
            avgToolsPerSession: round(totalTools / agg.sessionCount, 1),
          }
        : null,
  };
  ...
  if (cascade && cascade.pillars) {
    ...
    if (inputShare > 0.1) personality.push("input-heavy");
    if (outputShare > 0.1) personality.push("output-pusher");
  ...
  for (const item of iterationFingerprint) {
    if (item.correctionLoops > 0) {
      ...
      correctionTaxonomy[cat].maxDepth = Math.max(
        correctionTaxonomy[cat].maxDepth,
        item.maxLoopDepth,
      );
    }
  }
  ...
  const taxonomySorted = Object.entries(correctionTaxonomy)
    .map(([cat, data]) => ({
      category: cat,
      ...data,
      avgLoopsPerFile: round(data.totalLoops / data.files, 1),
    }))
  ...
  const designIterationIndex =
    fileEntries.length > 0 && agg.totalEdits > 0
      ? round(fileEntries[0][1] / agg.totalEdits, 4)
      : 0;
  const alignmentScore = round(
    se.acceptanceRate *
      (1 - Math.min(0.5, agg.correctionLoops.length / (agg.totalTurns || 1))),
    4,
  );
...
function clusterDirectives(directives) {
  ...
  for (const d of directives) {
    ...
    else if (/larger|bigger|size|font|scale|prominent/.test(lower))
      key = "larger-numbers";
    else if (/smaller|compact|dense|tight/.test(lower)) key = "compact";
    else if (/color|gold|amber|dark|light|theme/.test(lower))
      key = "color-theme";
    ...
    else if (/click|link|button|interactive|preview/.test(lower))
      key = "interactive";
...
function summarizeCluster(key, items) {
  ...
  const freq =
    count > 10
      ? `(strong signal — ${count} directives)`
      : count > 3
        ? `(${count} directives)`
        : `(1 directive — low confidence)`;
  const map = {
    ...
    other:
      count > 2
        ? `${count} general feedback directives (not clustered)`
        : `general feedback (low frequency)`,
  };
...
function buildDesignPreferences(fingerprint, taxonomy, deepTaste, agg) {
  ...
  const designCats = taxonomy.filter((t) =>
    ["design", "ui-component", "styling"].includes(t.category),
  );
  if (designCats.length > 0) {
    const top = designCats[0];
    design.push(
      `iterates heavily on ${top.category} (${top.totalLoops} correction loops across ${top.files} files — primary design focus)`,
    );
  }
  // From iteration fingerprint: concentration
  if (fingerprint.length > 0 && fingerprint[0].editCount > 50) {
    design.push(
      `concentrated design iteration on ${fingerprint[0].file} (${fingerprint[0].editCount} edits, ${fingerprint[0].correctionLoops} loops)`,
    );
  }
  ...
  if (design.length === 0 && fingerprint.length > 0) {
    ...
    if (converging > diverging) {
      design.push(
        "design iterations converge (agent eventually matches taste)",
      );
    } else if (diverging > converging) {
      design.push(
        "design iterations diverge (agent struggles to match taste — consider feeding preferences at session start)",
      );
    }
  }
  return design.length > 0
    ? design
    : ["no strong design iteration patterns detected"];
}
...
function buildCodePreferences(agg, se) {
  ...
  if (agg.editSizes.length > 0) {
    ...
    if (med < 200)
      code.push(
        `prefers small targeted edits (median edit ~${Math.round(med)} chars)`,
      );
    else if (med > 1000)
      code.push(
        `prefers large rewrites (median edit ~${Math.round(med)} chars)`,
      );
    else code.push(`mixed edit sizes (median edit ~${Math.round(med)} chars)`);
  }
  if (agg.editDeltas.length > 0) {
    const avgDelta =
      agg.editDeltas.reduce((a, b) => a + b, 0) / agg.editDeltas.length;
    if (Math.abs(avgDelta) < 50)
      code.push("tweaks more than adds/removes (near-zero net delta)");
    else if (avgDelta > 0)
      code.push(
        `net adder (avg delta +${Math.round(avgDelta)} chars per edit)`,
      );
    else
      code.push(
        `net remover (avg delta ${Math.round(avgDelta)} chars per edit)`,
      );
  }
  if (agg.replaceAllCount > 0) {
    code.push(
      `convention-aware (${agg.replaceAllCount} replace_all edits — global changes)`,
    );
  }
  const rejectRate =
    agg.totalToolUses > 0 ? agg.totalRejections / agg.totalToolUses : 0;
  if (rejectRate < 0.01) {
    code.push(
      `high acceptance (${round(rejectRate * 100, 2)}% reject rate — agent output usually matches taste)`,
    );
  } else if (rejectRate > 0.05) {
    code.push(
      `corrective (${round(rejectRate * 100, 1)}% reject rate — agent often misses taste)`,
    );
  }
  return code;
}
...
function buildWorkflowPreferences(rhythm, agg) {
  ...
  if (rhythm.primaryTool) {
    workflow.push(
      `primary tool: ${rhythm.primaryTool.name} (${rhythm.primaryTool.share}% of ${agg.totalToolUses} tool uses)`,
    );
  }
  workflow.push(rhythm.style);
  if (rhythm.askUserFrequency.calls > 0) {
    workflow.push(
      `prefers to be consulted (${rhythm.askUserFrequency.calls} AskUserQuestion calls, ${rhythm.askUserFrequency.perTurn}% of turns)`,
    );
  } else {
    workflow.push(
      "prefers autonomy (zero AskUserQuestion calls — lets the agent decide)",
    );
  }
  return workflow;
}
