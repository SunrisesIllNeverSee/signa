...
process.on("unhandledRejection", (reason) => {
  process.stderr.write(
    `[signa-mcp] unhandledRejection: ${reason?.stack || reason}\n`,
  );
  process.exit(1);
});
...
async function buildCtx(useCache = true) {
  ...
  const sessions = await readAllSessions(undefined, {
    maxFiles: 500,
    sinceDays: 30,
  });
  ...
  const taste = extractTaste(agg, settings.codename || "local", {
    cascade: cas,
  });
...
const TOOLS = [
  {
    name: "signa_scan",
    description:
      "Read your AI coding session logs locally, compute the full token cascade (Υ Yield, SNR, Leverage, Velocity, class), Steering Efficiency (SE), Appropriate Steering Index (ASI), and taste profile. Returns a summary of your current cascade state. All data stays local.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "signa_diagnose",
    description:
      "Diagnose your token cascade. Shows your class (BASE/POWER/ARCHITECT/TRANSMITTER), pillar breakdown, and flags issues (low leverage, low velocity, high input, etc.). Also shows SE and ASI.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "signa_simulate",
    description:
      'Simulate a pillar change and project the resulting Υ Yield and class. Shows the delta from your current state. Example: "increase output by 50000000" or "reduce input by 50%".',
    inputSchema: {
      type: "object",
      properties: {
        change: {
          type: "string",
          description:
            'The change to simulate. Format: "<pillar> <operator> <value>" where pillar is input/output/cacheRead/cacheCreate, operator is +/-/=, value is a number or percentage. Examples: "output +50000000", "input -50%", "cacheRead =100000000".',
        },
      },
      required: ["change"],
    },
    ...
    inputSchema: {
      ...
      properties: {
        deepTaste: {
          type: "boolean",
          description:
            "If true, include Layer 3 content-based preference extraction (opt-in, reads message content). Default false.",
        },
      },
    },
  },
  {
    name: "signa_asi",
    description:
      "Show your Appropriate Steering Index (ASI v2) — 8 dimensions measuring whether your interventions were the RIGHT ones, not just how often you accepted. Based on Anthropic autonomy research. Includes acceptance rate, correction precision, intervention timing, reliance slope, over-correction index, and under-steering index. Each dimension reports confidence level.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "signa_bridge",
    description:
      "The taste → cascade bridge: coaching insights connecting your behavioral taste profile to your cascade performance. This is the coaching no other tool can give — it requires both the taste profile AND the cascade formula. Returns insights sorted by severity (high/medium/low), each with observation, cascade impact, and recommendation.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "signa_cost",
    description:
      "Show your token-to-cost analysis. Computes total cost based on Claude pricing (input, output, cache create, cache read rates). Shows cost breakdown and cache savings (how much you saved by using cache vs fresh input).",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "signa_goal",
    description:
      "Show the path to a target class. Computes what pillar changes are needed to reach the target class (BASE/POWER/ARCHITECT/ARCHITECT+/TRANSMITTER) from your current state.",
    inputSchema: {
      type: "object",
      properties: {
        target: {
  ...
  },
  {
    name: "signa_compare",
    description:
      "Compare your cascade against a class average or a specific operator. Shows where you stand relative to the benchmark.",
    inputSchema: {
      type: "object",
      properties: {
        against: {
          type: "string",
          description:
            'What to compare against: a class name (POWER, ARCHITECT, TRANSMITTER) or "average".',
        },
      },
    },
  },
  {
    name: "signa_track",
    description:
      "Show your cascade metrics over time from local history. Displays trend lines for Υ, SNR, Leverage, Velocity, and class changes across recorded snapshots.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "signa_anomaly",
    description:
      "Detect metric drops and anomalies in your cascade history. Pinpoints when a metric dropped and by how much, helping you identify which session caused the change.",
    inputSchema: { type: "object", properties: {} },
  },
];
...
async function callTool(name, args = {}) {
  ...
  if (!ctx.agg) {
    return {
      error:
        "No session logs found. Run signa scan first or ensure ~/.claude/projects/ has .jsonl files.",
    };
  }
...
function toolDiagnose(ctx) {
  ...
  if (cas.leverage !== null && cas.leverage < 10) {
    issues.push({
      severity: "high",
      issue: `Leverage ${cas.leverage}× is LOW — not reusing enough context. Increase cache reads by loading prior context instead of re-typing.`,
    });
  }
  if (cas.velocity !== null && cas.velocity < 0.5) {
    issues.push({
      severity: "high",
      issue: `Velocity ${cas.velocity} is LOW — producing less output per unit of fresh input. Let the agent generate more before steering.`,
    });
  }
  if (cas.snr !== null && cas.snr < 0.5) {
    issues.push({
      severity: "medium",
      issue: `SNR ${cas.snr} is LOW — too much fresh input. Compress prompts or lean on cache.`,
    });
  }
  const p = cas.pillars;
  if (pct(p.input, p.total) > 0.1) {
    issues.push({
      severity: "medium",
      issue: `Fresh input is ${pct(p.input, p.total)} of total — high. The cascade rewards cache reuse over fresh instruction.`,
    });
  }
  if (cas.leverage > 100 && cas.velocity < 1) {
    issues.push({
      severity: "high",
      issue: `High leverage (${cas.leverage}×) but low velocity (${cas.velocity}) — hoarding context but not generating enough output. Push the agent to produce more.`,
    });
  }
  if (asi.asi !== null && asi.asi < 0.5) {
    issues.push({
      severity: "medium",
      issue: `ASI ${asi.asi} is low — your steering may be over-accepting or over-correcting. Check the ASI dimensions for specifics.`,
    });
  }
  return {
    class: cas.class,
...
function toolSimulate(ctx, changeStr) {
  ...
  if (!changeStr)
    return {
      error:
        'Missing "change" parameter. Example: "output +50000000" or "input -50%".',
    };
  ...
  const match = changeStr.match(
    /^(input|output|cacheRead|cacheCreate)\s*([+\-=])\s*([\d.]+)(%?)$/i,
  );
  if (!match) {
    return {
      error: `Could not parse change: "${changeStr}". Format: "<pillar> <operator> <value>" e.g. "output +50000000" or "input -50%"`,
    };
  }
  ...
  return {
    ...
    delta: {
      yield: Math.round(result.yield - cas.yield),
      classes:
        result.class !== cas.class
          ? `${cas.class} → ${result.class}`
          : "no change",
    },
...
function toolSuggest(ctx) {
  ...
  if (cas.leverage < 10) {
    suggestions.push({
      action:
        "Increase cache reads by loading prior context instead of re-typing",
      impact: "Leverage ↑ → Υ ↑",
      effort: "low",
      severity: "high",
    });
  ...
  if (cas.velocity < 1) {
    ...
    if (needed > 0) {
      ...
      suggestions.push({
        action: `Increase output by ${fmt(needed)} tokens — ask the agent for complete implementations, not pieces`,
        impact: `Velocity ↑ → Υ ${Math.round(simResult.yield)} (from ${cas.yield})`,
        effort: "medium",
        severity: "high",
      });
    }
  ...
  if (pct(cas.pillars.input, cas.pillars.total) > 0.05) {
    suggestions.push({
      action:
        "Compress prompts — reference prior context instead of re-explaining",
      impact: "Input ↓ → Υ ↑ (input is squared in denominator)",
      effort: "low",
      severity: "medium",
    });
  }
  ...
  if (asi.asi !== null && asi.asi < 0.7) {
    ...
    if (overCorr && overCorr.value > 0.3) {
      suggestions.push({
        action:
          "Reduce unnecessary corrections — before correcting, ask if you're fixing or adding. If adding, let the agent do it next turn.",
        impact: "Input ↓ → Υ ↑",
        effort: "low",
        severity: "medium",
      });
    ...
    if (timing && timing.value !== null && timing.value < 5) {
      suggestions.push({
        action:
          "Let the agent finish a complete unit of work before intervening. Batch your feedback.",
        impact: "Fewer context resets → cache ↑ → Υ ↑",
        effort: "low",
        severity: "low",
      });
    }
...
function toolTaste(ctx, deepTaste) {
  const { taste } = ctx;
  if (!taste)
    return { error: "No taste profile available. Run signa_scan first." };
  if (deepTaste && !taste.deepTaste) {
    // Re-extract with deep taste enabled
    const { agg, cas, settings } = ctx;
    const deepTasteProfile = extractTaste(agg, settings.codename || "local", {
      deepTaste: true,
      cascade: cas,
    });
    return {
...
function toolBridge(ctx) {
  ...
  if (!taste || !cas)
    return {
      error: "No taste or cascade data available. Run signa_scan first.",
    };
...
function toolGoal(ctx, target) {
  ...
  if (targetY === undefined)
    return {
      error: `Unknown class: ${target}. Valid: BASE, POWER, ARCHITECT, ARCHITECT+, TRANSMITTER`,
    };
  ...
  const neededOutput = Math.ceil(
    (targetY * cas.pillars.input ** 2) / cas.pillars.cacheRead,
  );
  const outputDelta = neededOutput - cas.pillars.output;
  // Also compute via input reduction
  const neededInput = Math.sqrt(
    (cas.pillars.cacheRead * cas.pillars.output) / targetY,
  );
  ...
  return {
    ...
    paths: [
      {
        action: `Increase output by ${fmt(outputDelta)} tokens`,
        from: cas.pillars.output,
        to: neededOutput,
        resultYield: targetY,
        effort: "medium",
      },
      {
        action: `Reduce input by ${fmt(inputDelta)} tokens (${pct(inputDelta, cas.pillars.input)} reduction)`,
        from: cas.pillars.input,
        to: Math.ceil(neededInput),
        resultYield: targetY,
        effort: "high",
      },
      {
        action: "Combination: increase output 50% + reduce input 20%",
        resultYield: simulate(cas, {
          output: cas.pillars.output * 1.5,
          input: cas.pillars.input * 0.8,
        }).yield,
        effort: "medium",
      },
    ],
...
async function toolTrack(ctx) {
  ...
  if (history.length === 0)
    return {
      error: "No history yet. Run signa_scan to record your first snapshot.",
    };
  return {
    ...
    trend:
      history.length > 1
        ? {
            yieldDelta:
              history[history.length - 1].cascade?.yield -
              history[0].cascade?.yield,
            classProgression: history
              .map((h) => h.cascade?.class)
              .filter(Boolean),
          }
        : null,
  };
...
async function toolAnomaly(ctx) {
  const history = await loadHistory();
  if (history.length < 2)
    return { error: "Need at least 2 snapshots to detect anomalies." };
  const anomalies = [];
