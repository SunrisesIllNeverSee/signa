...
function matchSkill(input) {
  ...
  // Direct commands
  if (lower === 'scan' || lower === 'rescan' || lower === 'refresh') return { skill: 'scan' }
  if (lower === 'diagnose' || lower === 'audit' || lower === 'diag') return { skill: 'diagnose' }
  if (lower.startsWith('simulate') || lower.startsWith('what if')) return { skill: 'simulate', args: lower.replace(/^simulate\s*/, '').replace(/^what if\s*/, '') }
  ...
  if (/how am i|how's it going|status|how do/.test(lower)) return { skill: 'diagnose' }
  if (/what should|recommend|suggest|improve|better/.test(lower)) return { skill: 'suggest' }
  if (/taste|preference|style|my profile/.test(lower)) return { skill: 'taste' }
  if (/cost|spend|dollar|money|price/.test(lower)) return { skill: 'cost' }
  if (/trend|history|over time|improving|declining/.test(lower)) return { skill: 'track' }
  return null // unknown
}
    ...
    try {
      switch (match.skill) {
        ...
        case 'watch':
          ...
          const root = ctx.settings.root || join(homedir(), '.claude', 'projects')
