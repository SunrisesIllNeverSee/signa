...
export async function parseSession(path) {
  ...
  for (const evt of events) {
    ...
    if (usage && typeof usage === "object") {
      ...
      message.tokens.cacheCreate = Number(
    ...
    if (ts) {
      if (!session.timeBounds.start || ts < session.timeBounds.start)
        session.timeBounds.start = ts;
      if (!session.timeBounds.end || ts > session.timeBounds.end)
        session.timeBounds.end = ts;
    }
  }
  ...
  for (const msg of session.messages) {
    ...
    for (const tu of msg.toolUses) {
      session.toolNameCounts[tu.name] =
        (session.toolNameCounts[tu.name] || 0) + 1;
      ...
      if (tu.name === "Bash" && tu.input?.command) {
        ...
        if (first)
          session.commandFirstWords[first] =
            (session.commandFirstWords[first] || 0) + 1;
      }
      if (
        tu.name === "Edit" ||
        tu.name === "Write" ||
        ...
        if (file) {
          session.fileEditCounts[file] =
            (session.fileEditCounts[file] || 0) + 1;
        }
        if (tu.name === "Edit") {
    ...
    if (msg.role === "user" && msg.textContent) {
      ...
      if (text.length > 5 && text.length < 500) {
        const imperative =
          /^(get rid of|make|remove|add|change|move|don't|do not|stop|use|prefer|want|need|fix|update|replace|hide|show|increase|decrease|larger|smaller|bigger)\b/i;
        if (
  ...
  for (const msg of session.messages) {
    for (const tu of msg.toolUses) {
      if (
        ...
        editSequence.push({
          file: tu.input.file_path,
          ts: msg.ts,
          delta:
            (tu.input.new_string || "").length -
            (tu.input.old_string || "").length,
        });
      }
    }
  }
  ...
  while (i < editSequence.length) {
    ...
    if (runLen >= 3) {
      ...
      const converging =
        ...
        deltas
          .slice(1)
          .every((d, k) => Math.abs(d) <= Math.abs(deltas[k]) + 50);
    ...
    if (msg.role === "assistant") {
      ...
      if (!turn.rejected && turn.userResponse) {
        ...
        if (assistantEditFiles.size > 0) {
          ...
          for (
            let m = k + 1;
            m < Math.min(k + 6, session.messages.length);
            m++
          ) {
            ...
            if (next.role === "user" || next.role === "assistant") {
              for (const tu of next.toolUses) {
                if (
                  (tu.name === "Edit" || tu.name === "MultiEdit") &&
                  assistantEditFiles.has(tu.input?.file_path)
                ) {
                  turn.corrected = true;
                  turn.accepted = false;
...
export function aggregateSessions(sessions) {
  ...
  for (const s of sessions) {
    ...
    agg.totalToolUses += Object.values(s.toolNameCounts).reduce(
      (a, b) => a + b,
      0,
    );
    agg.totalEdits += Object.values(s.fileEditCounts).reduce(
      (a, b) => a + b,
      0,
    );
    ...
    if (
      s.timeBounds.start &&
      (!agg.timeBounds.start || s.timeBounds.start < agg.timeBounds.start)
    ) {
      agg.timeBounds.start = s.timeBounds.start;
    }
    if (
      s.timeBounds.end &&
      (!agg.timeBounds.end || s.timeBounds.end > agg.timeBounds.end)
    ) {
